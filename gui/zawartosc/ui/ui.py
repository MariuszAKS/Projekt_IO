...
class Ui(Ui_MainWindow):
    """
    Klasa łącząca wszystkie elementu UI i ich funkcje
    """
    ...
    def __init__(self, glowne_okno: GlowneOkno) -> None:
        ...
        self.__menedzer_czcionki = MenedzerCzcionki(self.__glowne_okno)
        ...
        self.__dodaj_akcje_menu()

        self.__glowne_okno.ustaw_gdy_zamkniete(self.__analizator.watki.clear)

    def __pobierz_i_analizuj_zdjecia(self) -> None:
        '''Otwiera okno do wyboru plików do poddania analize, a następnie je analizuje'''
    ...
    def __dodaj_akcje_przyciskow(self) -> None:
        '''Ustawia jaką akcje ma wykonywać każdy przycisk'''
    ...
    def __dodaj_akcje_menu(self) -> None:
        '''Ustawia jaką akcję ma wykonywać każdy przycisk w menu'''

        self.akcja_systemowy.triggered.connect(self.__menedzer_motywow.ustaw_systemowy)
        self.akcja_systemowy.triggered.connect(self.__ladowacz_ikon.zaladuj_ciemne_ikony)
    ...
    def __dezaktywuj_przyciski(self) -> None:
        '''Wyłącza możliwośc używania przycisków na bocznym pasku'''

        self.przycisk_dodaj.setDisabled(True)
        self.przycisk_wyczysc.setDisabled(True)
        self.przycisk_eksport.setDisabled(True)

    def __aktywuj_przyciski(self) -> None:
        '''Włącza możliwośc używania przycisków na bocznym pasku'''
