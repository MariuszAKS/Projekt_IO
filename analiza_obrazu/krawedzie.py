...
class Krawedzie:
    ...
    def sredni_kolor_krawedzi(self):
        czerwony=self.obr_kolor[:,:,0]
        zielony=self.obr_kolor[:,:,1]
        niebieski=self.obr_kolor[:,:,2]
        return [czerwony[self.maska_krawedzi].mean(),zielony[self.maska_krawedzi].mean(),niebieski[self.maska_krawedzi].mean()]

    #zwraca odchylenie standardowe koloru krawedzi dla kazdego kanalu [czerwony,zielony,niebieski]
    def std_kolor_krawedzi(self):
        czerwony=self.obr_kolor[:,:,0]
        zielony=self.obr_kolor[:,:,1]
        niebieski=self.obr_kolor[:,:,2]
        return [czerwony[self.maska_krawedzi].std(),zielony[self.maska_krawedzi].std(),niebieski[self.maska_krawedzi].std()]
